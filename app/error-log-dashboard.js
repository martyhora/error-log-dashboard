...
const drawBasic = (chartData, selector, errorTitle, logType, onLabelClick) => {
    ...
    data.addRows(chartData.map(error => {
        return [error.date, parseInt(error.cnt), error.cnt, parseInt(error.uniqCnt), error.uniqCnt];
    }));
    ...
    const chart = new google.visualization.ColumnChart(document.getElementById(selector));

    google.visualization.events.addListener(chart, 'click', (e) => {
        const match = e.targetID.match(/hAxis#\d#label#(\d)/);

        if (match !== null && match.length) {
            const rowIndex = parseInt(e.targetID.substr(e.targetID.lastIndexOf('#') + 1));
...
const drawChart = (chartData, selector, errorTitle, logType, onLabelClick) => {
    ...
    google.charts.setOnLoadCallback(() => { drawBasic(chartData, selector, errorTitle, logType, onLabelClick) });
};
...
const drawBasicPieChart = (chartData, selector, errorTitle, onLabelClick) => {
    ...
    const chart = new google.visualization.PieChart(document.getElementById(selector));
...
const vm = {
    data: {
        countData: [
            ...
            { caption: 'Exception', logType: 'exception', data: [], log: [], errorTypes: [] },
        ],
        menu: [
            { page: 'trend', title: 'Trend', callback: null, },
        ],
    ...
    created: function() {
        ...
        this.countData.forEach(data => {
            this.menu.push({ page: `log-${data.logType}`, title: `${data.caption} log`, callback: null });
        });
    },
    methods: {
        fetchErrorCountData: function(logType) {
            axios.get(`${API_BASE}/errors/by-date/${this.project}/${logType}/${this.hideResolvedParam}/1`)
                .then(response => {
                    ...
                    this.countData.filter(log => log.logType === logType)[0].errorTypes = response.data.errorTypes;
                });
        },
    ...
    watch: {
        countData: {
            handler: function() {
                this.countData.forEach(data => {
                    drawChart(this.countData.filter(log => log.logType === data.logType)[0].data, `${data.logType}Chart`, data.caption, data.logType, (date, logType) => {
                        this.updateDayLog({ date: date, logType: data.logType });
                    });

                    drawPieChart(this.countData.filter(log => log.logType === data.logType)[0].errorTypes, `${data.logType}Piechart`, data.caption, (errorType) => {
